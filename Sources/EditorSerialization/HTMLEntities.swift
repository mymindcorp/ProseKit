// The HTML5 named character references, all 2125 of them, as the WHATWG
// standard lists them (https://html.spec.whatwg.org/entities.json).
//
// This is data, not code: 2125 pairs written as a dictionary literal would be
// slow to type-check and unreadable to review, so they are packed into one
// string and unpacked once, on first use. It lives in its own file for the same
// reason.
//
// Each line is a name, a space, and the value's code points in hex. Hex rather
// than the characters themselves because two values *are* the separators —
// `&NewLine;` is U+000A and `&Tab;` is U+0009 — so writing them literally put a
// newline in the middle of a record and silently decoded `&NewLine;` to nothing.
// Hex also keeps this file pure ASCII, and gives multi-scalar values (93 of
// them, e.g. `&ngE;` is U+2267 U+0338) an obvious spelling.
//
// The table used to hold a couple of hundred entries chosen by hand, which meant
// `&Dcaron;` or `&boxDL;` in an otherwise ordinary document came out as literal
// text. Which entities a document uses isn't something we can predict, so the
// answer is to carry all of them.

extension HTMLParser {
    /// The longest name in the table, which bounds the search for an entity's ";".
    static let longestEntityName = 31

    /// Unpacked with a byte scan rather than `split(separator:)` and `String`
    /// slicing, which halves the one-time build the first parse to meet an
    /// entity pays for: about 0.45 ms against 0.83 ms, release, five runs each.
    static let namedEntities: [String: String] = {
        let bytes = Array(packedEntities.utf8)
        var table = [String: String](minimumCapacity: 2125)
        var i = 0
        while i < bytes.count {
            var nameEnd = i
            while nameEnd < bytes.count, bytes[nameEnd] != UInt8(ascii: " ") { nameEnd += 1 }
            let name = String(decoding: bytes[i..<nameEnd], as: UTF8.self)
            var value = ""
            var scalar: UInt32 = 0
            var digits = 0
            var j = nameEnd + 1
            while j <= bytes.count {
                let c = j < bytes.count ? bytes[j] : UInt8(ascii: "\n")
                if c == UInt8(ascii: " ") || c == UInt8(ascii: "\n") {
                    if digits > 0, let u = Unicode.Scalar(scalar) { value.unicodeScalars.append(u) }
                    scalar = 0
                    digits = 0
                    if c == UInt8(ascii: "\n") { break }
                } else {
                    scalar = scalar << 4 | UInt32(c <= 57 ? c - 48 : (c | 0x20) - 87)
                    digits += 1
                }
                j += 1
            }
    ...
    private static let packedEntities = """
AElig C6
AMP 26
Aacute C1
Abreve 102
