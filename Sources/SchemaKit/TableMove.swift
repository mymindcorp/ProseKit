...
/// Transpose a 2D array (flip rows ↔ columns).
public func transpose<T>(_ array: [[T]]) -> [[T]] {
    guard let first = array.first else { return [] }
    return first.indices.map { i in array.map { $0[i] } }
}
...
public func convertArrayOfRowsToTableNode(_ tableNode: Node, _ arrayOfNodes: [[Node?]]) -> Node {
    ...
    for rowIndex in 0..<map.height {
        ...
        for colIndex in 0..<map.width {
            ...
            guard let oldCell = tableNode.nodeAt(cellPos) else { continue }
            if let newCell = try? oldCell.type.createChecked(cell.attrs, content: cell.content, marks: cell.marks) {
                newCells.append(newCell)
            }
        }
        if let newRow = try? oldRow.type.createChecked(oldRow.attrs, content: Fragment.from(newCells), marks: oldRow.marks) {
            newRows.append(newRow)
        }
    }
    return (try? tableNode.type.createChecked(tableNode.attrs, content: Fragment.from(newRows), marks: tableNode.marks)) ?? tableNode
}
