...
- **Selection and Escape** — `EditorTextView` leaves Escape available to its host after dismissing any open suggestion menu or Find bar. Set `escapeKeyBindingEnabled = true` to route it to the editor keymap, whose default selects the enclosing node. Selected-text dragging is disabled so pointer drags can start a new selection; set `textDraggingEnabled = true` to enable text drag-out. Image dragging and incoming drops are independent of that setting.
